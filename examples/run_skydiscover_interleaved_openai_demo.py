...
Run:
    ...
    python examples/run_skydiscover_interleaved_openai_demo.py --preset gepa
...
live in SkyDiscover YAML under ``search.database``. They are **not** mirrored as individual ``bbo.run`` flags: pass a YAML via
``--config`` or use ``--preset`` to load a small example from ``examples/skydiscover_configs/``.

Full field lists match the dataclasses in the installed SkyDiscover package
(``skydiscover/config.py``):
``AdaEvolveDatabaseConfig``, ``EvoxDatabaseConfig`` / ``SearchConfig``, ``GEPANativeDatabaseConfig``, …
See the SkyDiscover package for upstream reference configs such as ``adaevolve.yaml``,
``evox.yaml``, and ``openevolve.yaml``.
...
Notes:
    - There is no ``alphaevolve`` search id in SkyDiscover; use ``adaevolve``.
