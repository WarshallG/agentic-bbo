...
- `bbo/algorithms/llm_based/`
  - `llambo.py`, `opro.py`
  - `skydiscover_interleaved.py` (including skydiscover's implemention of OpenEvolve, GEPA, EvoX, and AdaEvolve, ShinkaEvolve can be added as an external backend)
...
The **SkyDiscover interleaved** algorithm (`skydiscover_interleaved`) installs the vendored `bbo/algorithms/llm_based/skydiscover` package as an editable dependency:
...
1. **Install the vendored SkyDiscover package** (editable path dependency):
...
uv run python -m bbo.run \
  ...
  --skydiscover-search-type openevolve_native
```
...
A ready-made script that mirrors `examples/run_opro_openai_demo.py` — including **`--search-type`** for `openevolve_native`, `adaevolve`, `evox`, `gepa_native`, etc. — is at **`examples/run_skydiscover_interleaved_openai_demo.py`**.

**Method-specific hyperparameters** (AdaEvolve islands and intensity, GEPA merge/gating, EvoX co-evolution settings, beam width, …) are **not** exposed as separate `bbo.run` flags. Configure them in **SkyDiscover YAML** under `search.database` (and for EvoX, also top-level `search.switch_interval`, `search.share_llm`, etc.), exactly as in standalone SkyDiscover. Field names match the dataclasses in vendored `bbo/algorithms/llm_based/skydiscover/skydiscover/config.py` (e.g. `AdaEvolveDatabaseConfig`, `GEPANativeDatabaseConfig`). Longer reference templates live under `bbo/algorithms/llm_based/skydiscover/configs/` (`adaevolve.yaml`, `evox.yaml`, …).

For quick experiments, copy or extend the small repo-local examples in **`examples/skydiscover_configs/`** (`adaevolve_bbo.yaml`, `evox_bbo.yaml`, `gepa_native_bbo.yaml`, `openevolve_native_bbo.yaml`) and pass `--config`, or run the demo with **`--preset adaevolve`** / **`--preset evox`** / **`--preset gepa_native`** / **`--preset openevolve_native`** (loads the matching example YAML). The YAML `search.type` should match **`--search-type`**; if they disagree, SkyDiscover’s `apply_overrides` may instantiate a fresh default database for the CLI type and **drop** custom `search.database` values from the file.
...
- `--skydiscover-search-type` — e.g. `topk`, `adaevolve`, `evox`, `gepa_native` (must be supported by your installed SkyDiscover).
