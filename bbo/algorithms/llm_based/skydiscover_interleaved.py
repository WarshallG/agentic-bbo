...
def _maybe_apply_bbo_full_rewrite_template_dir(config: Any) -> None:
    ...
    if getattr(config, "diff_based_generation", True):
        return
    if getattr(config.search, "type", "") == "evox":
        return
...
class SkydiscoverInterleavedAlgorithm(ExternalOptimizerAdapter):
    ...
    async def _run_skydiscover_round(self) -> None:
        try:
            ...
            from skydiscover.config import apply_overrides, load_config
        ...
        apply_overrides(config, search=self.skydiscover_search_type)
        ...
        if not config.llm.models:
            ...
            config.llm.models = [LLMModelConfig(name=model_name)]
        ...
        await runner.run(iterations=self.skydiscover_round_iterations)
